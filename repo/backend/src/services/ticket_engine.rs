...
pub fn redeem_ticket(
    ...
    conn.execute(
        ...
        rusqlite::params![now_str, redeemed_by, undo_until, ticket_id],
    ).map_err(|e| e.to_string())?;
    ...
    let _ = crate::audit::chain::append_audit_log(
        conn, redeemed_by, redeemed_by_username, "REDEEM", "ticket", ticket_id,
...
pub fn undo_redemption(
    ...
    conn.execute(
        ...
        rusqlite::params![now_str, undone_by, reason, ticket_id],
    ).map_err(|e| e.to_string())?;

    // Update the most recent redemption record (subquery: ORDER BY on UPDATE not supported on older SQLite)
    conn.execute(
    ...
    // Audit log
    let _ = crate::audit::chain::append_audit_log(
        conn, undone_by, undone_by_username, "UNDO", "ticket", ticket_id,
